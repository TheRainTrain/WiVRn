...
### `encoder`
Default value: `nvenc` if Nvidia GPU and compiled with nvenc, for all other GPU: `vulkan` if supported and enabled at compile time, `vaapi` when compiled with ffmpeg, else `x264`.
